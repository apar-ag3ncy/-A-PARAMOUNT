...
- **Cinematic intros**: `DoorIntro` (home, once per session) is a **pre-rendered 9s film** (`/public/door/door-open.mp4` + 540p + poster; frames baked offline — renderer technique in git history `51f2ada`) with the brand text as a DOM overlay synced to the film's beat map; coordination via `pm:doors-open` at 7.6s; server-rendered `#pm-precover` masks boot and `#smooth-wrapper` is hidden during the show. Like the kalash: do NOT rebuild this as live DOM/WebGL animation — bake frames offline. `CinematicHero` self-draws the logo; first scroll intent hurries its intro so it never overlaps the scrub timeline.
