...
- **Cinematic intros**: `DoorIntro` (home, once per session — temple doors push inward, layers in `/public/door/`, coordination via `pm:doors-open` event) and `CinematicHero` (self-drawing logo). The 3D showcase is a photo-lathe of a real silver kalash (`/public/kalash/`, profile hand-measured; UV half-width deliberately inside the alpha bbox).
