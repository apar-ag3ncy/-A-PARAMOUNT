        ...
        if subdirs:
            for sub in subdirs:
                ...
                for i, f in enumerate(imgs):
                    ...
                    b = 0 if (manifest_only and os.path.exists(dst)) else encode(os.path.join(src, sub, f), dst)
                    total_bytes += b; total_files += 1
                    paths.append(f"/gallery/{slug}/{mslug}/{i:02d}.webp")
            ...
            for i, f in enumerate(imgs):
                ...
                b = 0 if (manifest_only and os.path.exists(dst)) else encode(os.path.join(src, f), dst)
                total_bytes += b; total_files += 1
                paths.append(f"/gallery/{slug}/all/{i:02d}.webp")
            if paths:
