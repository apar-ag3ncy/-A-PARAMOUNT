...
def main():
    ...
    # emit TS
    # Resolve every finish label the catalogue can show, using the single
    # matcher in this file. CategoryGallery imports the result instead of
    # keeping a second, drift-prone copy in TypeScript.
    catalog = open(os.path.join(ROOT, "src", "lib", "catalog.ts")).read()
    labels = set()
    for block in re.findall(r"\[\s*\d+,\s*\"[^\"]+\",\s*\"[^\"]+\",\s*\[([^\]]*)\]", catalog):
        for lab in re.findall(r"\"([^\"]+)\"", block):
            labels.add(lab)
    variant_icons = {lab: match_icon(lab) for lab in sorted(labels)}
    print(f"resolved {len(variant_icons)} catalogue finish labels -> icons")
